...
for s = 1:length(subjectList)
    ...
    try
        ...
        %% Detect available XSens streams for gait analysis
        if verboseOutput, fprintf('🦴 Detecting XSens streams for gait analysis...\n'); end
        
        % Find XSens streams in synchronized data
        xsensSegmIdx = [];
        xsensAngularIdx = [];
        
        for i = 1:length(streamsMerged)
            if contains(streamsMerged{i}.info.name, 'LinearSegmentKinematicsDatagram1', 'IgnoreCase', true)
                xsensSegmIdx = i;
                if verboseOutput, fprintf('   ✅ Found XSens segment data stream\n'); end
            elseif contains(streamsMerged{i}.info.name, 'AngularKinematicsDatagram1', 'IgnoreCase', true)
                xsensAngularIdx = i;
                if verboseOutput, fprintf('   ✅ Found XSens angular data stream\n'); end
            end
        end
        
        %% Run gait event detection using synchronized streams
        if verboseOutput, fprintf('   🚶 Running gait event detection...\n'); end
        
        % Call gait detection function with synchronized streams
        gaitEvents = [];
        try
            gaitEvents = detect_gait_events_advanced(streamsMerged, eeg, verboseOutput);
            
            % Add gait events to EEG structure if detected
            if ~isempty(gaitEvents)
                if isempty(eeg.event)
                    eeg.event = gaitEvents;
                else
                    eeg.event = [eeg.event, gaitEvents];
                end
                if verboseOutput, fprintf('   ✅ Added %d gait events to EEG\n', length(gaitEvents)); end
            else
                if verboseOutput, fprintf('   ⚠️ No gait events detected\n'); end
            end
        catch ME
            if verboseOutput
...
if verboseOutput
    ...
    if ~isempty(alignmentQuality)
        ...
        rsquared_vals = [alignmentQuality.r_squared];
        rmse_vals = [alignmentQuality.rmse];
        
        fprintf('   R-squared - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
               mean(rsquared_vals), min(rsquared_vals), max(rsquared_vals));
        fprintf('   RMSE - Mean: %.3fs, Min: %.3fs, Max: %.3fs\n', ...
               mean(rmse_vals), min(rmse_vals), max(rmse_vals));
        
        eegCounts = [alignmentQuality.files_processed];
        eegCounts = [eegCounts.eeg_files];
        xdfCounts = [alignmentQuality.files_processed];
        xdfCounts = [xdfCounts.xdf_files];
        pupilCounts = [alignmentQuality.files_processed];
        pupilCounts = [pupilCounts.pupil_folders];
...
function [matchedXdf, matchedCsv] = match_recording_events(xdfEvents, xdfTimes, csvEvents, csvTimes, verbose)
    ...
    matchedCsv = [];
        ...
        %% Step 7: Use WaS4_merge for comprehensive stream synchronization
        if verboseOutput, fprintf('🦴 Using WaS4_merge for stream synchronization...\n'); end
        ...
        try
            ...
            % Call WaS4_merge to get properly synchronized streams
            if verboseOutput, fprintf('   🔄 Calling WaS4_merge for subject %03d...\n', subjectNum); end
            [eegMerged, streamsMerged] = WaS4_merge(subjectNum, dataPath);
            ...
            for i = 1:length(streamsMerged)
                if contains(streamsMerged{i}.info.name, 'CenterOfMass1', 'IgnoreCase', true)
                    xsensCoMIdx = i;
                    if verboseOutput, fprintf('   🎯 Found synced XSens CoM stream: %s\n', streamsMerged{i}.info.name); end
                elseif contains(streamsMerged{i}.info.name, 'LinearSegmentKinematicsDatagram1', 'IgnoreCase', true)
                    xsensSegmIdx = i;
                    if verboseOutput, fprintf('   🎯 Found synced XSens Segment stream: %s\n', streamsMerged{i}.info.name); end
                elseif contains(streamsMerged{i}.info.name, 'AngularKinematicsDatagram1', 'IgnoreCase', true)
                    xsensAngularIdx = i;
                    if verboseOutput, fprintf('   🎯 Found synced XSens Angular stream: %s\n', streamsMerged{i}.info.name); end
                end
            end
        
        %% Step 7b: Implement gait event detection using synchronized streams
        if verboseOutput, fprintf('   🚶 Running gait event detection...\n'); end
        
        % Call gait detection function with synchronized streams
        try
            gaitEvents = detect_gait_events_advanced(streamsMerged, eegMerged, verboseOutput);
            
            % Add gait events to EEG structure
            if ~isempty(gaitEvents)
                eventIdx = length(eeg.event);
                for i = 1:length(gaitEvents)
                    eventIdx = eventIdx + 1;
                    eeg.event(eventIdx) = gaitEvents(i);
                end
            ...
            if verboseOutput
                fprintf('   ❌ Error in gait detection: %s\n', ME.message);
            end
        end
        
        % Use the merged EEG data from WaS4_merge (contains all synchronized channels)
        eeg = eegMerged;
        
        catch ME
            if verboseOutput
                fprintf('   ❌ Error in WaS4_merge integration: %s\n', ME.message);
                fprintf('   💡 Continuing without XSens data\n');
            end
        end
        
        %% Step 8: Create alignment plots
        if createPlots
            ...
            if verboseOutput
                ...
                % Process each recording session using WaS4_merge approach
                if isfield(xsensData, 'center_of_mass') && ~isempty(xsensData.center_of_mass)
                    if verboseOutput, fprintf('   🔄 Processing Center of Mass data...\n'); end
                    ...
                    for x = 1:length(xsensData.center_of_mass)
                        if istable(xsensData.center_of_mass{x}) && height(xsensData.center_of_mass{x}) > 0
                            ...
                            % Get XSens frame rate (assume 60 Hz if not available)
                            frameRate = 60;
                            if isfield(xsensData, 'info') && length(xsensData.info) >= x
                                try
                                    if istable(xsensData.info{x}) && height(xsensData.info{x}) >= 5
                            ...
                            % Create XSens time vector
                            numFrames = height(comTable);
                            xsensTimeLocal = (0:numFrames-1) / frameRate;

                            % identify com stream
                            for i = 1:length(streams)
                                if strcmpi(streams{1,i}.info.name,'CenterOfMass1')
                                    comStream = streams{1,i};
                                end
                            end
                            
                            % Use cross-correlation to align .mat data with XSens stream
                            if ~isempty(comStream)
                                % Cross-correlate Y-axis CoM data to find alignment
                                streamComY = comStream.time_series(2, :); % Y-axis (vertical)
                                matComY = comTable.CoM_pos_y;
                                
                                % Normalize both signals for better correlation
                                streamComY = (streamComY - mean(streamComY)) / std(streamComY);
                                matComY = (matComY - mean(matComY)) / std(matComY);
                                
                                % Find optimal alignment using cross-correlation
                                [xcorrVals, lags] = xcorr(matComY,streamComY);
                                [~, maxIdx] = max(xcorrVals);
                                bestLag = lags(maxIdx);
                                
                                if verboseOutput
                                    fprintf('   🔄 Cross-correlation alignment: lag = %d samples (r = %.3f)\n', ...
                                           bestLag, max(xcorrVals)/length(streamComY));
                                end
                                
                                % Use XSens stream timestamps as reference
                                xsensStreamTimes = comStream.time_stamps;
                                
                                % Apply lag correction and create aligned timestamps
                                if bestLag >= 0
                                    % Mat data starts later, use stream times starting from lag offset
                                    alignedStartIdx = max(1, bestLag + 1);
                                    alignedEndIdx = min(length(xsensStreamTimes), bestLag + numFrames);
                                    xsensTimeLSL = xsensStreamTimes(alignedStartIdx:alignedEndIdx);
                                else
                                    % Stream starts later, use stream times shifted
                                    alignedStartIdx = max(1, -bestLag + 1);
                                    alignedEndIdx = min(length(xsensStreamTimes), length(xsensStreamTimes) + bestLag);
                                    xsensTimeLSL = xsensStreamTimes(alignedStartIdx:alignedEndIdx);
                                end
                                
                                % Ensure we have matching lengths
                                maxLength = min(length(xsensTimeLSL), numFrames);
                                xsensTimeLSL = xsensTimeLSL(1:maxLength);
                                
                            else
                                % Fallback to linear interpolation if no stream found
                                if verboseOutput
                                    fprintf('   ⚠️ No CoM stream found, using linear interpolation\n');
                                end
                                xsensTimeStart = eegTimeStart;
                                xsensTimeEnd = xsensTimeStart + (numFrames - 1) / frameRate;
                                xsensTimeLSL = linspace(xsensTimeStart, xsensTimeEnd, numFrames);
                            end
                            
                            % Find overlap with EEG recording
                            timeBuffer = 1.0; % 1 second buffer
                            overlapMask = (xsensTimeLSL >= (eegTimeStart - timeBuffer)) & ...
                                         (xsensTimeLSL <= (eegTimeEnd + timeBuffer));
                            
                            if sum(overlapMask) > 10 % Need reasonable overlap
                                % Extract overlapping data
                                xsensTimesOverlap = xsensTimeLSL(overlapMask);
                                
                                % Add CoM position channels
                                comCols = {'CoM_pos_x', 'CoM_pos_y', 'CoM_pos_z'};
                                for c = 1:length(comCols)
                                    colName = comCols{c};
                                    if ismember(colName, comTable.Properties.VariableNames)
                                        % Ensure we don't exceed table length
                                        dataLength = min(length(overlapMask), height(comTable));
                                        adjustedMask = overlapMask(1:dataLength);
                                        comData = comTable.(colName)(adjustedMask);
                                        
                                        % Debug time ranges before interpolation
                                        if verboseOutput
                                            fprintf('     🔍 Debug interpolation for %s:\n', colName);
                                            fprintf('       XSens overlap times: %.3f to %.3f (%d samples)\n', ...
                                                   min(xsensTimesOverlap), max(xsensTimesOverlap), length(xsensTimesOverlap));
                                            fprintf('       EEG times: %.3f to %.3f (%d samples)\n', ...
                                                   min(eeg.times), max(eeg.times), length(eeg.times));
                                            fprintf('       Data values range: %.6f to %.6f\n', min(comData), max(comData));
                                            
                                            % Detect potential time unit issues
                                            eegRange = max(eeg.times) - min(eeg.times);
                                            xsensRange = max(xsensTimesOverlap) - min(xsensTimesOverlap);
                                            if eegRange > 1000 && xsensRange < 1000
                                                fprintf('       ⚠️ POTENTIAL ISSUE: EEG times in milliseconds, XSens in seconds?\n');
                                            elseif eegRange < 1000 && xsensRange > 1000
                                                fprintf('       ⚠️ POTENTIAL ISSUE: EEG times in seconds, XSens in milliseconds?\n');
                                            end
                                            
                                            % Check time overlap
                                            timeOverlap = (max(min(xsensTimesOverlap), min(eeg.times)) <= ...
                                                          min(max(xsensTimesOverlap), max(eeg.times)));
                                            fprintf('       Time ranges overlap: %s\n', mat2str(timeOverlap));
                                        end
                                        
                                        % Fix massive data length mismatch (XSens: ~600 samples, EEG: ~1.8M samples)
                                        comInterp = [];
                                        
                                        if verboseOutput
                                            dataLengthRatio = length(eeg.times) / length(comData);
                                            fprintf('       Data length ratio: %.0f:1 (EEG:XSens)\n', dataLengthRatio);
                                        end
                                        
                                        % Simple linear mapping: stretch XSens data across full EEG timeline
                                        if length(comData) >= 2
                                            % Create a linear mapping from XSens samples to EEG samples
                                            xsensIndices = linspace(1, length(comData), length(eeg.times));
                                            
                                            % Interpolate XSens data to match EEG sample count
                                            comInterp = interp1(1:length(comData), comData, xsensIndices, 'linear', 'extrap');
                                            
                                            if verboseOutput
                                                fprintf('       Using simple linear mapping across EEG timeline\n');
                                            end
                                        else
                                            % Fallback: repeat single value or mean
                                            if length(comData) == 1
                                                comInterp = repmat(comData, size(eeg.times));
                                            else
                                                comInterp = repmat(mean(comData, 'omitnan'), size(eeg.times));
                                            end
                                            
                                            if verboseOutput
                                                fprintf('       Using constant fill (insufficient data)\n');
                                            end
                                        end
                                        
                                        % Debug interpolation result
                                        if verboseOutput
                                            nanCount = sum(isnan(comInterp));
                                            validCount = sum(~isnan(comInterp));
                                            fprintf('       Interpolation result: %d valid, %d NaNs out of %d (%.1f%% valid)\n', ...
                                                   validCount, nanCount, length(comInterp), validCount/length(comInterp)*100);
                                        end
                                        
                                        % Ensure interpolated data matches EEG sample count
                                        if length(comInterp) ~= size(eeg.data, 2)
                                            if verboseOutput
                                                fprintf('   ⚠️ Size mismatch: EEG has %d samples, interpolated data has %d\n', ...
                                                       size(eeg.data, 2), length(comInterp));
                                            end
                                            % Resize to match EEG data
                                            if length(comInterp) > size(eeg.data, 2)
                                                comInterp = comInterp(1:size(eeg.data, 2));
                                            else
                                                % Pad with NaN if too short
                                                comInterp(end+1:size(eeg.data, 2)) = NaN;
                                            end
                                        end
                                        ...
                                        % Ensure unique channel label (don't double-prefix CoM)
                                        if startsWith(colName, 'CoM_')
                                            eeg.chanlocs(newChannelIdx).labels = colName; % Already has CoM prefix
                                        else
                                ...
                                % Add CoM velocity channels
                                comVelCols = {'CoM_vel_x', 'CoM_vel_y', 'CoM_vel_z'};
                                for c = 1:length(comVelCols)
                                    colName = comVelCols{c};
                                    if ismember(colName, comTable.Properties.VariableNames)
                                        % Ensure we don't exceed table length
                                        dataLength = min(length(overlapMask), height(comTable));
                                        adjustedMask = overlapMask(1:dataLength);
                                        comVelData = comTable.(colName)(adjustedMask);
                                        
                                        % Use same interpolation approach as position data
                                        comVelInterp = [];
                                        if length(comVelData) >= 2
                                            % Create a linear mapping from XSens samples to EEG samples
                                            xsensIndices = linspace(1, length(comVelData), length(eeg.times));
                                            
                                            % Interpolate XSens data to match EEG sample count
                                            comVelInterp = interp1(1:length(comVelData), comVelData, xsensIndices, 'linear', 'extrap');
                                        else
                                            % Fallback: repeat single value or mean
                                            if length(comVelData) == 1
                                                comVelInterp = repmat(comVelData, size(eeg.times));
                                            else
                                                comVelInterp = repmat(mean(comVelData, 'omitnan'), size(eeg.times));
                                            end
                                        end
                                        
                                        % Ensure interpolated data matches EEG sample count
                                        if length(comVelInterp) ~= size(eeg.data, 2)
                                            if length(comVelInterp) > size(eeg.data, 2)
                                                comVelInterp = comVelInterp(1:size(eeg.data, 2));
                                            else
                                                comVelInterp(end+1:size(eeg.data, 2)) = NaN;
                                            end
                                        end
                                        
                                        % Add to EEG structure
                                        newChannelIdx = eeg.nbchan + 1;
                                        eeg.data(newChannelIdx, :) = comVelInterp;
                                        % Ensure unique channel label (don't double-prefix CoM)
                                        if startsWith(colName, 'CoM_')
                                            eeg.chanlocs(newChannelIdx).labels = colName; % Already has CoM prefix
                                        else
                                ...
                                if verboseOutput
                                    totalRecordingTime = eegTimeEnd - eegTimeStart;
                                    overlapTime = max(xsensTimesOverlap) - min(xsensTimesOverlap);
                                    overlapPercent = overlapTime / totalRecordingTime * 100;
                                    fprintf('   ✅ Added CoM channels (%.1f%% overlap, %d samples)\n', ...
                                           overlapPercent, sum(overlapMask));
                                end
                            else
                                if verboseOutput
                                    fprintf('   ⚠️ Insufficient CoM data overlap with EEG\n');
                                end
                            end
                        end
                    end
                ...
                % Process Foot Position data
                if isfield(xsensData, 'segment_position') && ~isempty(xsensData.segment_position)
                    if verboseOutput, fprintf('   🔄 Processing Foot Position data...\n'); end
                    ...
                    for x = 1:length(xsensData.segment_position)
                        if istable(xsensData.segment_position{x}) && height(xsensData.segment_position{x}) > 0
                            ...
                            if sum(overlapMask) > 10
                                xsensTimesOverlap = xsensTimeLSL(overlapMask);
                                
                                % Find foot and toe position columns
                                ...
                                % Add foot position channels
                                for f = 1:length(footCols)
                                    colName = footCols{f};
                                    % Ensure we don't exceed table length
                                    dataLength = min(length(overlapMask), height(segTable));
                                    adjustedMask = overlapMask(1:dataLength);
                                    footData = segTable.(colName)(adjustedMask);
                                    
                                    % Use same interpolation approach as CoM data
                                    footInterp = [];
                                    if length(footData) >= 2
                                        % Create a linear mapping from XSens samples to EEG samples
                                        xsensIndices = linspace(1, length(footData), length(eeg.times));
                                        
                                        % Interpolate XSens data to match EEG sample count
                                        footInterp = interp1(1:length(footData), footData, xsensIndices, 'linear', 'extrap');
                                    else
                                        % Fallback: repeat single value or mean
                                        if length(footData) == 1
                                            footInterp = repmat(footData, size(eeg.times));
                                        else
                                            footInterp = repmat(mean(footData, 'omitnan'), size(eeg.times));
                                        end
                                    end
                                    
                                    % Ensure interpolated data matches EEG sample count
                                    if length(footInterp) ~= size(eeg.data, 2)
                                        if length(footInterp) > size(eeg.data, 2)
                                            footInterp = footInterp(1:size(eeg.data, 2));
                                        else
                                            footInterp(end+1:size(eeg.data, 2)) = NaN;
                                        end
                                    end
                                    ...
                                    % Ensure unique channel label with XSens prefix
                                    eeg.chanlocs(newChannelIdx).labels = sprintf('XSens_%s', colName);
                                    eeg.chanlocs(newChannelIdx).type = 'XSens';
                                    eeg.nbchan = eeg.nbchan + 1;
                                    channelsAdded = channelsAdded + 1;
                                end
                                
                                % Add toe position channels
                                for t = 1:length(toeCols)
                                    colName = toeCols{t};
                                    % Ensure we don't exceed table length
                                    dataLength = min(length(overlapMask), height(segTable));
                                    adjustedMask = overlapMask(1:dataLength);
                                    toeData = segTable.(colName)(adjustedMask);
                                    
                                    % Use same interpolation approach as CoM data
                                    toeInterp = [];
                                    if length(toeData) >= 2
                                        % Create a linear mapping from XSens samples to EEG samples
                                        xsensIndices = linspace(1, length(toeData), length(eeg.times));
                                        
                                        % Interpolate XSens data to match EEG sample count
                                        toeInterp = interp1(1:length(toeData), toeData, xsensIndices, 'linear', 'extrap');
                                    else
                                        % Fallback: repeat single value or mean
                                        if length(toeData) == 1
                                            toeInterp = repmat(toeData, size(eeg.times));
                                        else
                                            toeInterp = repmat(mean(toeData, 'omitnan'), size(eeg.times));
                                        end
                                    end
                                    
                                    % Ensure interpolated data matches EEG sample count
                                    if length(toeInterp) ~= size(eeg.data, 2)
                                        if length(toeInterp) > size(eeg.data, 2)
                                            toeInterp = toeInterp(1:size(eeg.data, 2));
                                        else
                                            toeInterp(end+1:size(eeg.data, 2)) = NaN;
                                        end
                                    end
                                ...
                                if verboseOutput && (~isempty(footCols) || ~isempty(toeCols))
                                    totalRecordingTime = eegTimeEnd - eegTimeStart;
                                    overlapTime = max(xsensTimesOverlap) - min(xsensTimesOverlap);
                                    overlapPercent = overlapTime / totalRecordingTime * 100;
                                    fprintf('   ✅ Added %d foot/toe channels (%.1f%% overlap)\n', ...
                                           length(footCols) + length(toeCols), overlapPercent);
                                end
                            end
                        end
                    end
                ...
                %% Add PhotoSensor from XDF
                try
                    photoIdx = cellfun(@(x) contains(x.info.name, 'Photo', 'IgnoreCase', true), streams);
                    if any(photoIdx)
                        if verboseOutput, fprintf('   📷 Adding PhotoSensor channel from XDF...\n'); end
                        
                        photoStream = find(photoIdx, 1);
                        ampPhoto = interp1(streams{photoStream}.time_stamps, double(streams{photoStream}.time_series), counterXdfTimes, 'pchip');
                        
                        newChannelIdx = eeg.nbchan + 1;
                        eeg.data(newChannelIdx, :) = NaN(1, size(eeg.data, 2));
                        eeg.data(newChannelIdx, idxAmp) = ampPhoto;
                        eeg.chanlocs(newChannelIdx).labels = 'PhotoSensor';
                        eeg.chanlocs(newChannelIdx).type = 'PhotoSensor';
                        eeg.nbchan = newChannelIdx;
                        
                        if verboseOutput
                            fprintf('   ✅ Added PhotoSensor channel\n');
                        end
                    else
                        if verboseOutput
                            fprintf('   ⚠️ No PhotoSensor stream found in XDF\n');
                        end
                    end
                catch ME
                    if verboseOutput
                        fprintf('   ❌ Error adding PhotoSensor: %s\n', ME.message);
                    end
                end
